...
> ⚠️ **Do not use `k8s.io/cluster-autoscaler/...` Azure tags.** Azure rejects tags containing `/` as `InvalidTagNameCharacters`. The Kubernetes Cluster Autoscaler on Azure does **not** discover VMSS via those tags — it uses the `--nodes=min:max:VMSS_NAME` flag on its own deployment. See [configure-cluster-autoscaler.md](configure-cluster-autoscaler.md) and [vmss-for-kubernetes.md — Cluster Autoscaler Integration](../concepts/vmss-for-kubernetes.md#how-the-cluster-autoscaler-integrates-with-vmss) for the correct setup.
