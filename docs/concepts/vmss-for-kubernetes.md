...
1. **Discovery**: CA discovers VMSS node pools through the autoscaler's own configuration — either explicit `--nodes=<min>:<max>:<vmss-name>` flags on the CA deployment, or `--node-group-auto-discovery=label:cloudprovider.kubernetes.io/auto-discovery=<value>` for tag-based discovery on the VMSS.

   > ⚠️ **Do not try to set Azure resource tags like `k8s.io/cluster-autoscaler/enabled=true`.** Azure rejects tags containing `/` (and `<`, `>`, `%`, `&`, `\`, `?`) as `InvalidTagNameCharacters`. These tag names appear in some unofficial guides but are an AWS Cluster Autoscaler convention — they do not work on Azure.
   >
   > Azure's CA discovery options are:
   > - **Explicit** (recommended): `--nodes=1:10:vmss-k8s-workers` on the CA Deployment
   > - **Auto-discovery via valid Azure tags**: `--node-group-auto-discovery=label:cloudprovider.kubernetes.io/auto-discovery=cluster1` plus matching tag on the VMSS (the tag value must use Azure-safe characters)

2. **Scale-out**: When pods are pending due to insufficient resources, CA calls the Azure VMSS scale API to increase the instance count.
