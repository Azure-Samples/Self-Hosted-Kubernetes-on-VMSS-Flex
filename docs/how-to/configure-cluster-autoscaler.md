...
> 📝 **Discovery convention:** This guide uses Azure-safe tags and `--nodes` flags. **Do not use** `k8s.io/cluster-autoscaler/*` tags — those are an AWS convention; Azure rejects tags containing `/` as `InvalidTagNameCharacters`. See [vmss-for-kubernetes.md — Cluster Autoscaler Integration](../concepts/vmss-for-kubernetes.md#how-the-cluster-autoscaler-integrates-with-vmss) for context.

> 📝 **Instance names:** Legacy examples below use `vm-controlplane-01/02/03`. On a Flex VMSS deployment ([main quickstart](../quickstart/deploy-kubeadm-vmss.md)), real names look like `vmss-k8s-controlplane_4fc08081`. Enumerate via `az vm list -g <rg> --query "[?contains(name,'controlplane')].name" -o tsv`.
...
- A worker VMSS (no special tags required for the `--nodes` discovery method)
...
> ⚠️ **The `k8s.io/cluster-autoscaler/*` tag scheme is an AWS convention. Azure rejects these tags** with `InvalidTagNameCharacters` because Azure tag names cannot contain `/`. The Azure Cluster Autoscaler uses different mechanisms:
>
> 1. **Explicit `--nodes` flags** (recommended, simplest) — list the VMSS by name on the CA Deployment. No tags needed.
> 2. **Label-based auto-discovery** via `--node-group-auto-discovery=label:KEY=VALUE` — set a single Azure-safe tag on the VMSS (e.g. `cluster-autoscaler-enabled=true`) and reference it.
>
> Use **explicit `--nodes`** unless you have many VMSS pools and want CA to discover new ones automatically.

If you want auto-discovery, set Azure-safe tags on the worker VMSS:
...
az vmss update \
  ...
  --set tags.cluster-autoscaler-cluster=$CLUSTER_NAME
```

(Optional) Hint CA about node resources for accurate scale-from-zero planning:

```bash
az vmss update \
  --resource-group $RESOURCE_GROUP \
  --name $WORKER_VMSS_NAME \
  --set tags.cluster-autoscaler-cpu=8 \
  --set tags.cluster-autoscaler-memory=32Gi
```
      ...
      containers:
        - image: registry.k8s.io/autoscaling/cluster-autoscaler:v1.29.3
          ...
          command:
            ...
            # Recommended: explicit nodes flag (one per VMSS pool)
            - --nodes=1:10:vmss-k8s-workers
            # Alternative: auto-discovery via Azure-safe labels (uncomment if used)
