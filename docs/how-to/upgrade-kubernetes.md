...
> 📝 **Instance names:** This guide uses `vm-controlplane-01/02/03` as conceptual placeholders for the three control-plane nodes. On a Flex VMSS deployment (what the [main quickstart](../quickstart/deploy-kubeadm-vmss.md) produces), real names look like `vmss-k8s-controlplane_4fc08081`. Substitute as you run each command, or enumerate via `az vm list -g <rg> --query "[?contains(name,'controlplane')].name" -o tsv` and pick the first.
